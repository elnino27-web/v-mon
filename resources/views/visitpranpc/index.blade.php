...
@section('dashboard_title', 'VISIT PRANPC MONITORING')
...
@section('content')
    ...
    <div class="grid grid-cols-1 lg:grid-cols-4 gap-6 mb-6 items-stretch">
        ...
        <div class="col-span-1 flex flex-col gap-4 h-full">
            <div class="flex-1 bg-blue-600 rounded-2xl p-5 text-white shadow-lg shadow-blue-500/20 transform transition-all duration-500 hover:-translate-y-1 hover:shadow-xl relative overflow-hidden group flex flex-col justify-center">
                <div class="relative z-10">
                    <p class="text-xs font-bold tracking-widest uppercase text-white/80 mb-1">Total Data Visit</p>
                    <h3 class="font-display font-black text-4xl counter-animate" id="val_total">0</h3>
                </div>
                <svg class="absolute -right-4 -bottom-4 w-24 h-24 text-white/20 transform group-hover:scale-110 transition-transform duration-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 11H5m14 0a2 2 0 012 2v6a2 2 0 01-2 2H5a2 2 0 01-2-2v-6a2 2 0 012-2m14 0V9a2 2 0 00-2-2M5 11V9a2 2 0 002-2m0 0V5a2 2 0 012-2h6a2 2 0 012 2v2M7 7h10"></path></svg>
            </div>
            ...
            <div class="flex-1 bg-[#10b981] rounded-2xl p-5 text-white shadow-lg shadow-emerald-500/20 transform transition-all duration-500 hover:-translate-y-1 hover:shadow-xl relative overflow-hidden group flex flex-col justify-center">
                <div class="relative z-10">
                    <p class="text-xs font-bold tracking-widest uppercase text-white/80 mb-1">Total Lunas (Paid)</p>
            ...
            <div class="flex-1 bg-t-red rounded-2xl p-5 text-white shadow-lg shadow-t-red/20 transform transition-all duration-500 hover:-translate-y-1 hover:shadow-xl relative overflow-hidden group flex flex-col justify-center">
                <div class="relative z-10">
                    <p class="text-xs font-bold tracking-widest uppercase text-white/80 mb-1">Belum Lunas (Not Paid)</p>
        ...
        <div class="col-span-1 lg:col-span-3 bg-white rounded-2xl shadow-sm border border-slate-100 p-6 md:p-8 flex flex-col justify-center relative h-full" id="capture_area">
            <div class="flex justify-between items-start border-b border-slate-100 pb-4 mb-6">
                <div class="flex items-center gap-3">
                    ...
                    <h2 class="font-display font-bold text-xl text-slate-800">Rasio Realisasi Pelunasan (Visit)</h2>
                </div>
            ...
            <div class="mb-8 flex-grow">
                <div class="flex justify-between text-sm font-bold text-slate-500 mb-2 uppercase tracking-wide">
                    <span>Sisa Belum Lunas (Not Paid)</span>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-6">
        ...
        <div class="col-span-1 bg-white rounded-2xl shadow-sm border border-slate-100 p-6 flex flex-col">
            <div class="flex items-center gap-2 mb-2">
                ...
                <h3 class="font-display font-bold text-lg text-slate-800">Status Kunjungan (Activity)</h3>
            </div>
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden mb-8">
        <div class="p-6 border-b border-slate-100 bg-slate-50/50">
            <div class="flex items-center gap-2 mb-5">
                ...
                <h2 class="font-display font-bold text-lg text-slate-800">Detail Transaksi Visit PRANPC</h2>
            </div>

            <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
                <div>
                    <label class="block text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-1">Pencarian Global</label>
                    <div class="relative">
                        <div class="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                            <svg class="w-4 h-4 text-slate-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"></path></svg>
                        </div>
                        <input type="text" id="filter_global" placeholder="Cari data..." class="w-full pl-9 pr-3 py-2 border border-slate-200 rounded-xl text-sm focus:outline-none focus:border-t-red focus:ring-1 focus:ring-t-red transition-all">
                    </div>
                </div>
                <div>
                    <label class="block text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-1">Filter STO</label>
                    <select id="filter_sto" class="w-full px-3 py-2 border border-slate-200 rounded-xl text-sm focus:outline-none focus:border-t-red transition-all appearance-none bg-white">
                        <option value="">Semua STO</option>
                    </select>
                </div>
                <div>
                    <label class="block text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-1">Respon (Reason)</label>
                    <select id="filter_reason" class="w-full px-3 py-2 border border-slate-200 rounded-xl text-sm focus:outline-none focus:border-t-red transition-all appearance-none bg-white">
                        <option value="">Semua Reason</option>
                    </select>
                </div>
                <div>
                    <label class="block text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-1">Status Billing</label>
                    <select id="filter_billing" class="w-full px-3 py-2 border border-red-200 bg-red-50 text-t-red font-semibold rounded-xl text-sm focus:outline-none focus:border-t-red transition-all appearance-none">
                        <option value="">Semua Status</option>
                        <option value="PAID">Paid (Lunas)</option>
                        <option value="NOTPAID">Not Paid (Belum Lunas)</option>
                    </select>
                </div>
            </div>
        </div>
...
@section('scripts')
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            ...
            // Pembersih spasi Regex
            const getColExact = (name) => headers.findIndex(h => h && h.trim().replace(/\s+/g, ' ').toUpperCase() === name.replace(/\s+/g, ' ').toUpperCase());

            // Kolom Spesifik Visit PRANPC
            const idxBilling = getColExact('Status Billing');
            const idxStatus = getColExact('Activity Status');
            const idxReason = getColExact('Activity Reason');
            const idxSTO = getColExact('STO');

            // Kolom Tambahan untuk Tampilan Default
            const idxSND = getColExact('SND');
            const idxNamaPT = getColExact('Nama Perusahaan');
            const idxAgent = getColExact('Nama Agent');
            const idxAddress = getColExact('Address');
            const idxDate = getColExact('Tgl Visit');
            ...
            let countBertemu = 0;
            let countTidakBertemu = 0;

            rows.forEach(row => {
                const r = row.map(cell => cell || '');

                // Hitung Status Billing (Hanya PAID dan NOTPAID)
                const billing = idxBilling !== -1 && r[idxBilling] ? r[idxBilling].toUpperCase().trim().replace(/\s/g, '') : '';
                if(billing === 'PAID') totalLunas++;
                else if (billing === 'NOTPAID') totalBelumLunas++;

                // Hitung Activity Status
                const activity = idxStatus !== -1 && r[idxStatus] ? r[idxStatus].trim() : '';
                if(activity.toUpperCase() === 'BERTEMU PELANGGAN') countBertemu++;
                else if (activity.toUpperCase() === 'TIDAK BERTEMU PELANGGAN') countTidakBertemu++;

                // Kumpulkan filter Dropdown
                if(idxSTO !== -1 && r[idxSTO] && r[idxSTO].trim() !== '-') stoSet.add(r[idxSTO].trim());
                if(idxReason !== -1 && r[idxReason] && r[idxReason].trim() !== '-') reasonSet.add(r[idxReason].trim());
            });
            ...
            // Progress Bar (Rasio Lunas vs Total Data Keseluruhan)
            const pctLunas = totalData > 0 ? Math.round((totalLunas / totalData) * 100) : 0;
            const pctBelumLunas = totalData > 0 ? Math.round((totalBelumLunas / totalData) * 100) : 0;
            const gap = totalData > 0 ? (100 - pctLunas) : 0;
            ...
            document.getElementById('txt_gap').innerText = gap;
            ...
            // Set default ke Activity Reason
            if (idxReason !== -1) chartSelector.value = idxReason;
            ...
            chartSelector.addEventListener('change', function() { updateBarChart(this.value); });
            ...
            new Chart(ctxActivity, {
                type: 'doughnut',
                data: {
                    labels: ['Bertemu Pelanggan', 'Tidak Bertemu Pelanggan'],
                    datasets: [{
                        data: [countBertemu, countTidakBertemu],
            ...
            // Kolom bawaan yang tampil berdasarkan request
            const rawVisibleCols = [idxSND, idxNamaPT, idxSTO, idxAgent, idxStatus, idxReason, idxAddress, idxBilling, idxDate];
            const defaultVisibleCols = rawVisibleCols.filter(i => i !== -1); // Filter membuang index yang tidak ditemukan
            ...
            const table = $('#mainTable').DataTable({
                ...
                buttons: [
                    { extend: 'colvis', text: '⚙️ Pilih Kolom', className: 'dt-custom-colvis' }
                ],
                columnDefs: headers.map((h, i) => ({
                    ...
                    render: function (data, type, row) {
                        // Badge untuk Status Billing
                        if (i === idxBilling && data) {
                            const val = data.toUpperCase().trim().replace(/\s/g, '');
                            if (val === 'PAID') return `<span class="px-2 py-1 bg-green-100 text-green-700 text-xs font-bold rounded-md">PAID</span>`;
                            if (val === 'NOTPAID') return `<span class="px-2 py-1 bg-red-100 text-red-700 text-xs font-bold rounded-md">NOT PAID</span>`;
                        }
                        // Badge untuk Activity Status
                        if (i === idxStatus && data) {
                            if (data.toUpperCase() === 'BERTEMU PELANGGAN') return `<span class="text-green-600 font-bold">✓ Bertemu</span>`;
                            if (data.toUpperCase() === 'TIDAK BERTEMU PELANGGAN') return `<span class="text-red-500 font-semibold">✗ Tidak Bertemu</span>`;
                        }
                        return data || '-';
                    }
                })),
                language: {
                    info: "Menampilkan _START_ sampai _END_ dari _TOTAL_ data visit",
                    paginate: { next: "Next &raquo;", previous: "&laquo; Prev" },
                    emptyTable: "Data visit tidak ditemukan"
                }
            });

            // Isi Dropdown Filter secara Alfabetis
            Array.from(stoSet).sort().forEach(v => $('#filter_sto').append(new Option(v, v)));
            Array.from(reasonSet).sort().forEach(v => $('#filter_reason').append(new Option(v, v)));

            // Event Listeners Filter
            $('#filter_global').on('keyup', function () { table.search(this.value).draw(); });

            $('#filter_sto').on('change', function () {
                let val = $.fn.dataTable.util.escapeRegex(this.value);
                if(idxSTO !== -1) table.column(idxSTO).search(val ? '^' + val + '$' : '', true, false).draw();
            });
            $('#filter_reason').on('change', function () {
                let val = $.fn.dataTable.util.escapeRegex(this.value);
                if(idxReason !== -1) table.column(idxReason).search(val ? '^' + val + '$' : '', true, false).draw();
            });
            $('#filter_billing').on('change', function () {
                let val = $.fn.dataTable.util.escapeRegex(this.value);
                if(idxBilling !== -1) {
                    // Karena data asli mungkin "Not Paid" (dengan spasi), pencarian regex kita sesuaikan
                    let searchRegex = '';
                    if(val === 'PAID') searchRegex = '^PAID$';
                    if(val === 'NOTPAID') searchRegex = '^(NOT PAID|NOTPAID)$'; // Cover dua variasi penulisan

                    table.column(idxBilling).search(val ? searchRegex : '', true, false).draw();
                }
            });
            ...
            document.getElementById('btn_download').addEventListener('click', function() {
                ...
                html2canvas(captureArea, { scale: 2, backgroundColor: "#ffffff", logging: false }).then(canvas => {
                    ...
                    link.download = 'Rasio_Pelunasan_Visit_CMON.png';
